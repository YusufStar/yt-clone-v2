...
const CreateVideoModalComp = ({ setModal }) => {
  ...
  const createVideo = () => {
    if(videoDesc !== "" && videoName !== "" && miniPic) {
      console.log(
        {
          video: imageToUrl(videoFile),
          miniPicture: imageToUrl(miniPic),
          videoName: videoName,
          videoDesc: videoDesc,
        }
      )
  ...
  return (
    <div className="fixed left-0 top-0 w-screen h-screen flex items-center justify-center z-50 py-[50px] px-[500px]">
      <div className="w-full h-full rounded-xl bg-[#282828] z-[999] flex flex-col">
        {videoFile ? (
          <>
            ...
            <div className="w-full h-full p-10 flex flex-col gap-3">
              ...
              <div className="flex flex-col gap-3 mb-auto">
                ...
                {miniPic ? (
                  <>
                  <img src={imageToUrl(miniPic)} className="w-[200px] h-[100px] rounded-xl"/>
                  </>
                ):(
                  <>
                  <button onClick={() => {
	 ...
	 M21,7h-2v2h-2V7h-2V5h2V3h2v2h2V7z M13,4v6v1h1h6v9H4V4H13 M14,3H3v18h18V10h-7V3L14,3z"
                  ...
                  <input onChange={(e) => setMiniPic(e.target.files[0])} id="minipic" type="file" className="hidden" accept="image/png, image/gif, image/jpeg"/>
                </button>
                </>
                )}
              </div>
              ...
              <div className="w-full h-auto py-2 px-2 flex items-center justify-end">
                <button onClick={createVideo} className="bg-[#3ea6ff] px-3 py-2 rounded-md text-black text-md hover:opacity-80 font-semibold">Gönder</button>
              </div>
            </div>
          </>
        ) : (
          <>
            ...
            >
              ...
              <div className="px-5 py-2 bg-[#3ea6ff] rounded-md mt-3 z-[9999] cursor-pointer">
                <button {...getInputProps()} className=""></button>
                Dosya Seç
              </div>
            </div>
          </>
